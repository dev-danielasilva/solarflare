...
function EditTopicDialog({ open, topic, handleClose, courseid, subjectid, sendUpdateTopic }) {
	...
	const [hasDefaultImage, setHasDefaultImage] = useState<boolean>(topic.image as boolean);
	...
	function onSubmit({ name, description, image }: InferType<typeof schema>) {
		if (!image && !hasDefaultImage) {
			triggerError('Debes añadir una imagen para el tema. Agrega una y prueba de nuevo.');
			return;
		}
	...
	function updateTopic(topicDefinition) {
		setIsLoading(true);
		jwtService
			.updateTopic(topic.id, topicDefinition, courseid)
			.then((response: any) => {
				setIsLoading(false);
				sendUpdateTopic(response.data);
			})
			.catch((e) => {
				setIsLoading(false);
				dispatch(
